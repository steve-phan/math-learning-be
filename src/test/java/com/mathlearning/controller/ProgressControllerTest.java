...
class ProgressControllerTest {
    ...
    void getProgress_ReturnsProgressDto() throws Exception {
        when(userProgressRepository.findByUserId(anyLong())).thenReturn(Optional.of(testProgress));
        when(submissionRepository.countByUserIdAndIsCorrect(anyLong(), anyBoolean()))
                .thenReturn(10L) // total
                .thenReturn(8L); // correct

        mockMvc.perform(get("/api/progress")
                .with(authentication(
                        new org.springframework.security.authentication.TestingAuthenticationToken(1L, null))))
    ...
    void getProgress_NewUser_ReturnsDefaultProgress() throws Exception {
        when(userProgressRepository.findByUserId(anyLong())).thenReturn(Optional.empty());
        when(submissionRepository.countByUserIdAndIsCorrect(anyLong(), anyBoolean())).thenReturn(0L);
    ...
    void getMistakes_ReturnsMistakeList() throws Exception {
        when(mistakeNotebookRepository.findByUserIdAndReviewed(anyLong(), anyBoolean()))
